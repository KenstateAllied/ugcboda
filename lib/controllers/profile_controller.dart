...
class ProfileController extends GetxController {
  ...
  Future<void> fetchuserProfile() async {
    ...
    if (!NetworkUtils.checkInternet()) {
      return; // Stop if there's no connectivity
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        */
        ...
        userProfile.value = UserProfile.fromJson(
            data); // update user profile with the JSON data from UserProfile model
        ...
        logger.warning(
            'Profile request failed (${response.statusCode}): ${response.body}');
        SnackbarUtils.showSnackbar(
          title: 'Oh snap! 😔',
      ...
      SnackbarUtils.showSnackbar(
        title: 'Oh snap! 😔',
        message:
            "Couldn't fetch your profile. Please check your internet connection.",
        contentType: ContentType.warning,
      );
    } finally {
      isLoading(false);
  ...
  Future<void> updateProfile() async {
    ...
    final request =
        http.MultipartRequest('PUT', Uri.parse(ApiConstants.editUserProfile));
    request.headers['Authorization'] = 'Bearer $token';
    request.fields['profile_data'] =
        jsonEncode(profileData); // Add profile data as a JSON string

    // Attach image if selected
    if (selectedImage.value != null) {
      String extension = selectedImage.value!.path
          .split('.')
          .last
          .toLowerCase(); // Get the image file extension
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        SnackbarUtils.showSnackbar(
            ...
            contentType: ContentType.warning);
        throw Exception(
            '[ERROR]: $responseMessage | Status code: ${response.statusCode}');
      }
